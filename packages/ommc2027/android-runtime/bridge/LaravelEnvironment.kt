...
class LaravelEnvironment(private val context: Context) {
    ...
    companion object {
        ...
        private const val STARTUP_MIGRATION_COMMAND = "app:startup-migrate --no-interaction"
    ...
    private fun runStartupMigrationGate() {
        val output = phpBridge.runArtisanCommand(STARTUP_MIGRATION_COMMAND)
        val status = startupMigrationValue(output, "STARTUP_MIGRATION_STATUS")

        if (status == null) {
            throw RuntimeException("Startup migration gate returned no status. Output: $output")
        }

        Log.d(TAG, "🚀 Startup migration gate status: $status")

        if (status == "failed") {
            throw RuntimeException(startupMigrationFailureDetail(output))
        }
    }

    private fun startupMigrationValue(output: String, key: String): String? {
    ...
    private fun startupMigrationFailureDetail(output: String): String {
        val dbPath = startupMigrationValue(output, "STARTUP_MIGRATION_DB_PATH") ?: "unknown"
        val failedMigration = startupMigrationValue(output, "STARTUP_MIGRATION_FAILED_MIGRATION")
        val errorClass = startupMigrationValue(output, "STARTUP_MIGRATION_ERROR_CLASS")
        val errorMessage = startupMigrationValue(output, "STARTUP_MIGRATION_ERROR_MESSAGE")
            ?: "Unknown migration failure."

        val migrationSuffix = failedMigration?.let { " Migration: $it." } ?: ""
        val classSuffix = errorClass?.let { " Error: $it." } ?: ""

        return "Database migration failed for $dbPath.$migrationSuffix$classSuffix $errorMessage"
    }
...
"""
    ...
    fun initializeForBackground() {
        try {
            ...
            if (!phpBridge.nativeIsPersistentRuntimeLive()) {
                runStartupMigrationGate()
            }
