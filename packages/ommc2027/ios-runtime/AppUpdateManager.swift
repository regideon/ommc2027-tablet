...
class AppUpdateManager {
    ...
    /// Bootstrap script required before PHP may launch (must match NativePHPApp SCRIPT_FILENAME).
    static let requiredBootstrapRelativePath = "vendor/nativephp/mobile/bootstrap/ios/native.php"

    private let documentsPath: String
    private let appPath: String
    private let updatesPath: String
    private(set) var appReady = false
    ...
    func bootstrapFilePath() -> String {
        return (appPath as NSString).appendingPathComponent(Self.requiredBootstrapRelativePath)
    }

    func isExtractedAppValid() -> Bool {
        return isValidApp(at: appPath)
    }
    ...
    func ensureAppExists() -> Bool {
        ...
        let extractedIsValid = isValidApp(at: appPath)

        if !extractedIsValid {
            print("📦 Extracted app missing or incomplete — re-extracting from bundle")
            if copyBundledApp() {
                didExtract = true
            }
        } else if shouldUpdateFromBundle() {
            print("📦 Bundled version differs — re-extracting from bundle")
            if copyBundledApp() {
                didExtract = true
            }
        } else {
            print("📦 An app bundle has already been extracted and validated")
        }
        ...
        appReady = isValidApp(at: appPath)

        if appReady {
            print("✅ App is ready (didExtract=\(didExtract))")
        } else {
            let message = lastExtractionError ?? "Required NativePHP bootstrap files are missing after extraction"
            print("❌ App is NOT ready: \(message)")
        }

        return didExtract
    }

    /// Force a clean re-extract from the embedded app.zip (used for one-shot recovery).
    @discardableResult
    func forceReextractFromBundle() -> Bool {
        print("📦 Forcing re-extraction from bundled app.zip")
        lastExtractionError = nil
        let success = copyBundledApp()
        appReady = isValidApp(at: appPath)
        return success && appReady
    }

    /// Atomically extract bundled app.zip into Documents/app after validating required files.
    @discardableResult
    private func copyBundledApp() -> Bool {
        ...
        guard let bundleZipPath = Bundle.main.path(forResource: "app", ofType: "zip") else {
            lastExtractionError = "No bundled app.zip found in the application package"
            print("❌ \(lastExtractionError!)")
            return false
        }

        let extractPath = updatesPath + "/extract_" + UUID().uuidString
        ...
        do {
            ...
            // Prefer streaming unzip to avoid buffering ~190MB in memory (jetsam risk).
            do {
                try FileManager.default.unzipItem(at: sourceURL, to: extractURL)
            } catch {
                print("⚠️ unzipItem failed (\(error)); falling back to parallel extract")
                try? FileManager.default.removeItem(at: extractURL)
                try FileManager.default.createDirectory(at: extractURL, withIntermediateDirectories: true)
                try extractZipParallel(from: sourceURL, to: extractURL)
            }

            guard isValidApp(at: extractPath) else {
                lastExtractionError = "Extracted app.zip is missing required bootstrap/runtime files"
                print("❌ \(lastExtractionError!)")
                try? FileManager.default.removeItem(atPath: extractPath)
                return false
            }

            // Swap into place only after validation. Keep the previous app until the new one is ready.
            let swapBackupPath = updatesPath + "/swap_backup_" + UUID().uuidString
            if FileManager.default.fileExists(atPath: appPath) {
                try FileManager.default.moveItem(atPath: appPath, toPath: swapBackupPath)
            }

            do {
                try FileManager.default.moveItem(atPath: extractPath, toPath: appPath)
            } catch {
                // Restore previous app if swap fails
                if FileManager.default.fileExists(atPath: swapBackupPath) {
                    try? FileManager.default.moveItem(atPath: swapBackupPath, toPath: appPath)
                }
                throw error
            }
            ...
            lastExtractionError = "Failed to extract bundled app: \(error.localizedDescription)"
            print("❌ \(lastExtractionError!)")
            try? FileManager.default.removeItem(atPath: extractPath)
            return false
        }
    ...
    private func isValidApp(at path: String) -> Bool {
        // Root `artisan` is intentionally stripped from app.zip; NativePHP uses
        // vendor/nativephp/mobile/bootstrap/ios/artisan.php for CLI instead.
        let requiredRelativePaths = [
            ".env",
            "vendor/autoload.php",
            "bootstrap/app.php",
            "public/index.php",
            Self.requiredBootstrapRelativePath,
        ...
        for relativePath in requiredRelativePaths {
            ...
            if !FileManager.default.fileExists(atPath: fullPath) {
                print("❌ Missing required extracted file: \(relativePath)")
                return false
            }
        }
